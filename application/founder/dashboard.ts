...
export async function loadFounderDashboard():Promise<FounderDashboardModel>{
  ...
  const activation=obj(snapshot.activation),discovery=obj(snapshot.discovery),research=obj(snapshot.research),evidence=obj(snapshot.evidence),truth=obj(snapshot.truth),r4=obj(snapshot.r4),r5=obj(snapshot.r5),r6=obj(snapshot.r6),opportunity=obj(snapshot.opportunity),engagement=obj(snapshot.engagement),ai=obj(snapshot.ai);
