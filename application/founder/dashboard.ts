...
export async function loadFounderDashboard():Promise<FounderDashboardModel>{
  ...
  const stages:FounderStage[]=[
    {key:"growth",label:env.growthEnabled?"Genesis database growth":"Genesis intelligence bank",technicalLabel:env.growthEnabled?"Autonomous intelligence builder":"Demand-driven product policy",state:env.growthEnabled?(runtimeState(snapshot,"GROWTH",8)==="ATTENTION"||num(growth.actionsFailed)>0?"ATTENTION":runtimeState(snapshot,"GROWTH",8)):"DISABLED",value:num(growth.companies),valueLabel:"reusable intelligence companies",detail:env.growthEnabled?`${num(growth.dense80)} at ≥80% data density · ${num(growth.dense100)} fully dense · ${text(growth.phase)||"SEED"} phase`:`Growth paused · existing bank retained · customer campaigns drive new research`,latestAt:iso(growth.latestAt),secondary:env.growthEnabled?`${money(num(growth.spentTodayUsd))} today`:"No speculative spend"},
