...
export class GenesisDatabaseGrowthService{
  ...
  private async discover(action:GrowthAction){
    const industryKey=action.industryKey!;const industryName=INDUSTRIES[industryKey]??industryKey;const existing=await this.repo.existingDomains(industryKey,5000);
    const result=await this.ai.discoverIndustry({industryKey,industryName,count:action.discoveryBatchSize,existingDomains:existing});let persisted=0,coreComplete=0,evidenceRows=0;
    for(const c of result.value.companies){const companyId=await this.repo.ensureCompany({industryKey,name:c.name,domain:c.domain,websiteUrl:c.websiteUrl,countryCode:c.countryCode,discoveryReason:c.discoveryReason});const keys=new Set<string>();for(const row of c.evidence){await this.persistClaim(companyId,row);if(row.polarity==="SUPPORTS")keys.add(row.claimKey);evidenceRows++;}await this.evaluateCompany(companyId);const complete=["identity.canonical_name","identity.canonical_domain","operation.current"].every(k=>keys.has(k));await this.repo.markStage(companyId,"CORE",complete,complete?null:"GENESIS_GROWTH_CORE_EVIDENCE_INCOMPLETE",complete?null:retryAt(action.retryHours),new Date().toISOString());persisted++;if(complete)coreComplete++;}
  ...
  private async profile(action:GrowthAction){
    const company=await this.context.company(action.companyId!);const industryName=INDUSTRIES[action.industryKey??""]??action.industryKey??"Unclassified";const result=await this.ai.researchProfile({companyName:company.name,domain:company.canonicalDomain,industryName});const keys=new Set<string>();for(const row of result.value.findings){await this.persistClaim(company.companyId,row);if(row.polarity==="SUPPORTS")keys.add(row.claimKey);}await this.evaluateCompany(company.companyId);const core=["identity.canonical_name","identity.canonical_domain","operation.current"].every(k=>keys.has(k));const profileKeys=["industry.category","commercial.offering","market.customer","geography.primary","business_model.type","commercial.buying_signal"];const profileCount=profileKeys.filter(k=>keys.has(k)).length;const at=new Date().toISOString();await this.repo.markStage(company.companyId,"CORE",core,null,null,at);await this.repo.markStage(company.companyId,"PROFILE",profileCount>=3,profileCount>=3?null:"GENESIS_GROWTH_PROFILE_INCOMPLETE",profileCount>=3?null:retryAt(action.retryHours),at);return{costUsd:result.usage.estimatedCostUsd,metadata:{companyId:company.companyId,findings:result.value.findings.length,coreComplete:core,profileFields:profileCount,model:result.model,responseId:result.responseId}};
  }

  private async routes(action:GrowthAction){
    const company=await this.context.company(action.companyId!);const result=await this.ai.researchRoutes({companyName:company.name,domain:company.canonicalDomain});let persisted=0;
    for(const row of result.value.routes){if(row.polarity!=="SUPPORTS")continue;const companyNode={nodeKind:"COMPANY" as const,companyId:company.companyId,label:company.name};const access={tenantScopeOrganisationId:null,nodeKind:"ACCESS_POINT" as const,stableKey:`growth:${cleanKey(row.accessPointKind)}:${cleanKey(row.accessPointValue)}`,label:row.accessPointKind.replace(/_/g," "),accessPointKind:row.accessPointKind as AccessPointKind,canonicalValue:row.accessPointValue};if(row.organisationalUnitKey&&row.organisationalUnitLabel){const unit={tenantScopeOrganisationId:null,nodeKind:"ORGANISATIONAL_UNIT" as const,stableKey:`growth:${cleanKey(company.companyId)}:${cleanKey(row.organisationalUnitKey)}`,label:row.organisationalUnitLabel};await this.relationships.recordEvidence({relationship:{tenantScopeOrganisationId:null,relationType:"parent_of",from:companyNode,to:unit},source:source(row),acquisition:acquisition(row),evidence:evidence(row),polarity:"SUPPORTS",linkMethod:"AI_EXTRACTED",linkVersion:"MRV2-GENESIS-GROWTH-1.0.0"});await this.relationships.recordEvidence({relationship:{tenantScopeOrganisationId:null,relationType:"has_access_point",from:unit,to:access},source:source(row),acquisition:acquisition(row),evidence:evidence(row),polarity:"SUPPORTS",linkMethod:"AI_EXTRACTED",linkVersion:"MRV2-GENESIS-GROWTH-1.0.0"});persisted+=2;}else{await this.relationships.recordEvidence({relationship:{tenantScopeOrganisationId:null,relationType:"has_access_point",from:companyNode,to:access},source:source(row),acquisition:acquisition(row),evidence:evidence(row),polarity:"SUPPORTS",linkMethod:"AI_EXTRACTED",linkVersion:"MRV2-GENESIS-GROWTH-1.0.0"});persisted++;}}
    const complete=persisted>0;await this.repo.markStage(company.companyId,"ROUTES",complete,complete?null:"GENESIS_GROWTH_ROUTE_NOT_FOUND",complete?null:retryAt(action.retryHours),new Date().toISOString());return{costUsd:result.usage.estimatedCostUsd,metadata:{companyId:company.companyId,routesReturned:result.value.routes.length,relationshipsPersisted:persisted,complete,model:result.model,responseId:result.responseId}};
  }

  private async contactsAction(action:GrowthAction){
    const company=await this.context.company(action.companyId!);const result=await this.ai.researchContacts({companyName:company.name,domain:company.canonicalDomain});let qualified=0;
    for(const row of result.value.contacts){if([row.identityEvidence,row.employmentEvidence,row.roleEvidence,row.channelEvidence].some(e=>e.polarity!=="SUPPORTS"))continue;const personId=await this.repo.ensurePerson(company.companyId,row.name);const companyNode={nodeKind:"COMPANY" as const,companyId:company.companyId,label:company.name};const personNode={nodeKind:"PERSON" as const,personId,label:row.name};
      ...
      const access={tenantScopeOrganisationId:null,nodeKind:"ACCESS_POINT" as const,stableKey:`growth:${cleanKey(row.channelKind)}:${cleanKey(row.channelValue)}`,label:row.channelKind.replace(/_/g," "),accessPointKind:row.channelKind as AccessPointKind,canonicalValue:row.channelValue};const rel=await this.relationships.recordEvidence({relationship:{tenantScopeOrganisationId:null,relationType:"has_access_point",from:personNode,to:access},source:source(row.channelEvidence),acquisition:acquisition(row.channelEvidence),evidence:evidence(row.channelEvidence),polarity:"SUPPORTS",linkMethod:"AI_EXTRACTED",linkVersion:"MRV2-GENESIS-GROWTH-1.0.0"});
      await this.contacts.recordEvidence({claim:{kind:"CHANNEL_OWNERSHIP",tenantScopeOrganisationId:null,accessPointId:rel.to.node_id,personId},source:source(row.channelEvidence),acquisition:acquisition(row.channelEvidence),evidence:evidence(row.channelEvidence),polarity:"SUPPORTS",linkMethod:"AI_EXTRACTED",linkVersion:"MRV2-GENESIS-GROWTH-1.0.0"});qualified++;}
    const complete=qualified>0;await this.repo.markStage(company.companyId,"CONTACTS",complete,complete?null:"GENESIS_GROWTH_CONTACT_NOT_FOUND",complete?null:retryAt(action.retryHours),new Date().toISOString());return{costUsd:result.usage.estimatedCostUsd,metadata:{companyId:company.companyId,contactsReturned:result.value.contacts.length,qualifiedContacts:qualified,complete,model:result.model,responseId:result.responseId}};
  }
  ...
  async runCycle(){const settings=this.settings();await this.repo.syncSettings(settings);if(!settings.enabled)return{status:"DISABLED" as const,processed:0};const at=new Date().toISOString();let runId:string|null=null;let processed=0,succeeded=0,failed=0,budgetExhausted=false;try{runId=await this.repo.start(at);for(let i=0;i<settings.maxActions;i++){await this.repo.heartbeat(runId,new Date().toISOString());const next=await this.repo.nextAction(runId,new Date().toISOString());if(!next)break;if(next.state==="BUDGET_EXHAUSTED"){budgetExhausted=true;break;}processed++;try{const result=await this.execute(next);await this.repo.complete(next.actionRunId,result.costUsd,result.metadata,new Date().toISOString());succeeded++;}catch(error){await this.repo.fail(next.actionRunId,errorCode(error),0,retryAt(next.retryHours),new Date().toISOString());failed++;}}
      await this.repo.finish(runId,failed>0&&succeeded>0?"PARTIAL":failed>0?"FAILED":"SUCCEEDED",{processed,succeeded,failed,budgetExhausted},new Date().toISOString());return{status:"OK" as const,processed,succeeded,failed,budgetExhausted};
    }catch(error){if(runId){try{await this.repo.finish(runId,"FAILED",{errorCode:errorCode(error),processed,succeeded,failed},new Date().toISOString());}catch{}}throw error;}}
}
