...
export class AnonymousDiscoveryService{
  ...
  async status(browserSecret:string):Promise<AnonymousDiscoveryView|null>{
    ...
    const pipeline=anonymousPipelineFromStatus(raw);const narrative=await conversation.discovery(raw,pipeline);return{...raw,pipeline,currentMessage:narrative.summary||messageOf(raw,pipeline),narrative,freeOpportunityLimit:8,opportunities};
  }
}
