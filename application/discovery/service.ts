...
export function anonymousPipelineFromStatus(raw:AnonymousDiscoveryStatusRecord):AnonymousPipelineStage[]{
  ...
  return [
    stage("UNDERSTAND","Getting to know your business",understandDone,!understandDone,"I'm reading your website and learning exactly what you sell."),
    stage("MAP","Understanding your market",mapDone,understandDone&&!mapDone,"I'm working out which types of businesses are most likely to need what you offer."),
    stage("DISCOVER","Finding companies",discoverDone,mapDone&&!discoverDone,"I'm checking what MarketRoute already knows first, then finding new companies only where your market needs them.",m.scopedCompanies?`${m.scopedCompanies} found`:undefined),
    stage("RESEARCH","Researching the strongest candidates",researchDone,discoverDone&&!researchDone,"I'm gathering enough evidence to decide which companies are genuinely worth your time.",m.researchWorkTotal?`${m.researchWorkCompleted} / ${m.researchWorkTotal} research tasks`:m.researchedCompanies?`${m.researchedCompanies} researched`:undefined),
    stage("EVALUATE","Checking the opportunity",m.opportunities>0&&researchDone,evaluateStarted&&!routeStarted,"I'm looking for a real commercial reason to pursue each company.",m.opportunities?`${m.opportunities} opportunities`:undefined),
    stage("ROUTE","Finding the way in",ready,routeStarted&&!ready,"I'm finding the right people and checking which contact routes are genuinely usable.",m.structuralRoutes?`${m.structuralRoutes} routes`:undefined),
    stage("READY","Getting opportunities ready",ready,ready,"As soon as an opportunity is strong enough, it appears here and becomes one of your permanent free opportunities.",m.freeUnlocked?`${m.freeUnlocked} of 8 free`:undefined),
  ];
}
