...
export class OpenAIResponsesClient{
  ...
  async structured<T>(input:{name:string;schema:Record<string,unknown>;instructions:string;prompt:string;webSearch?:boolean;allowedDomains?:string[];signal?:AbortSignal;maxOutputTokens?:number}):Promise<OpenAIStructuredResult<T>>{
    const body:any={model:openAIModel(),instructions:input.instructions,input:input.prompt,reasoning:{effort:process.env.OPENAI_REASONING_EFFORT?.trim()||"low"},text:{format:{type:"json_schema",name:input.name,strict:true,schema:input.schema}},max_output_tokens:input.maxOutputTokens??4000};
    ...
    const raw=await response.text();let parsed:any;try{parsed=raw?JSON.parse(raw):null}catch{parsed={raw}}if(!response.ok)throw new Error(`MARKETROUTE_OPENAI_HTTP_${response.status}:${String(parsed?.error?.code??parsed?.error?.message??"REQUEST_FAILED")}`);
    ...
    return {value,responseId:String(parsed.id??""),model:String(parsed.model??openAIModel()),usage:usageOf(parsed),sourceUrls:sourceUrlsOf(parsed)};
  }
}
