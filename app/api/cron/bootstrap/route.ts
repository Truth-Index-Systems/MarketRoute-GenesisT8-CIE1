...
export async function GET(request:Request){try{assertCronRequest(request);const result=await observeProductionRuntime("BOOTSTRAP",()=>runBootstrapCron());return NextResponse.json({ok:true,...result});}catch(error){const message=error instanceof Error?error.message:"MARKETROUTE_BOOTSTRAP_CRON_FAILED";return NextResponse.json({ok:false,error:message},{status:message==="MARKETROUTE_CRON_UNAUTHORISED"?401:500});}}
