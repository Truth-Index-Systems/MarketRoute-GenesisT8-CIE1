...
export default async function CommandCentre(){
  ...
  return <div>
    ...
    <Panel>
      ...
      {campaigns.length===0?<EmptyState icon="campaigns" title="No campaigns yet" body="Create your first campaign through the platform setup flow before MarketRoute can begin researching a market."/>:<div className="mr-live-card-list">{campaigns.map((c)=><a className="mr-live-card" href={`/app/campaigns/${c.campaignId}`} key={c.campaignId}><div className="mr-live-card__main"><span className="mr-live-card__seller">{c.sellerName}</span><div className="mr-live-card__title"><strong>{c.name}</strong><StatusBadge compact label={humanStatus(c.workflowState)} title={c.workflowState} tone={c.workflowState==="ACTIVE"?"green":"slate"}/></div><p>{c.objectiveText??"Commercial objective pending."}</p></div><dl><div><dt>Companies</dt><dd>{c.scopedCompanies}</dd></div><div><dt>Ready to pursue</dt><dd>{countFromObject(c.dispositionCounts,"ACTIONABLE")}</dd></div><div><dt>Need review</dt><dd>{countFromObject(c.workflowCounts,"REVIEWABLE")}</dd></div><div><dt>Research today</dt><dd>{money(c.budget.spentTodayUsd)}</dd></div></dl><span className="mr-live-card__open">Open campaign →</span></a>)}</div>}
    </Panel>
  </div>
}
