...
export default async function Engagement({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <div>
    <PageHeader eyebrow="ENGAGEMENT" title="From ready opportunity" accent="to controlled outreach." description="Prepare and review outreach only after MarketRoute has established a current commercial case and contact route." actions={<CampaignSwitcher campaigns={campaigns.map(c=>({campaignId:c.campaignId,name:c.name,workflowState:c.workflowState}))} current={campaignId} action="/app/engagement"/>}/>
    <div className="mr-policy-banner"><div><span>Message approval policy</span><strong>{model.policyMode==="AUTOPILOT"?"Autopilot after approval":"Human approval required"}</strong></div><StatusBadge label={humanStatus(model.policyMode)} title={model.policyMode} tone={model.policyMode==="AUTOPILOT"?"violet":"blue"}/><p>Autopilot never bypasses opportunity approval or MarketRoute’s live send-time authority checks.</p></div>
