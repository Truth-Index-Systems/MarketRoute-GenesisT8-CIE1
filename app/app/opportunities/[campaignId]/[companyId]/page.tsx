...
export default async function OpportunityWorkspace({params,searchParams}:{params:Promise<{campaignId:string;companyId:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <div>
    ...
    <section className="mr-dashboard-grid mr-dashboard-grid--secondary">
      ...
      <Panel><SectionHeading eyebrow="READINESS" title={profile.executableNow?"MarketRoute has classed this opportunity ready":"MarketRoute is still resolving this opportunity"} description={profile.executableNow?"Readiness is system-owned: current commercial, route and contact authority are all present. You do not need to approve the opportunity before acting.":"The opportunity becomes ready automatically when the required research and authority are current."}/><ProvenanceTrail items={[{label:"Current state",value:profile.executableNow?"Ready":humanStatus(text(workflow.state,"Researching"))},{label:"Commercial authority",value:profile.authorityReady?"Current":"Not complete"},{label:"Outreach can be prepared",value:model.actions.canGenerateEngagement?"Yes":"Not yet"},{label:"Ready to act",value:model.actions.canExecute?"Yes":"Not yet"}]}/>{!canMutate&&<div className="mr-readonly-note"><Icon name="shield" size={15}/><span>Viewer access is read-only.</span></div>}<div className="mr-readonly-note"><Icon name="shield" size={15}/><span>MarketRoute decides readiness from current evidence. Human approval is reserved for the message you choose to send.</span></div></Panel>
