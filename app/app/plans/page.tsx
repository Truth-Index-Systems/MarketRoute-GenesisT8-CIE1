...
export default async function Plans({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <div><PageHeader eyebrow="PLAN & BILLING" title="Keep MarketRoute" accent="working your market." description="Choose your research capacity, or manage an existing subscription. Billing changes product access only; commercial intelligence remains evidence-led and demand-driven."/>
    ...
    {overCapacity&&<div className="mr-alert mr-alert--danger"><Icon name="warning" size={16}/><span>Your current plan allows {capacity.activeMarketLimit} active market{capacity.activeMarketLimit===1?"":"s"}, but this workspace has {capacity.activeMarketCount}. MarketRoute has paused research beyond the plan allowance without deleting any campaign data. Archive markets or upgrade to restore research capacity.</span></div>}
