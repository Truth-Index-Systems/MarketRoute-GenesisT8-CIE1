...
function billingMessage(code:string){
  ...
  if(code.includes("PLAN_CAMPAIGN_LIMIT_TOO_LOW"))return "That plan cannot hold all of your current markets. Choose a plan with more active markets.";
...
export default async function Plans({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  const overCapacity=(access.mode==="PAID"||access.mode==="FULL")&&capacity.activeMarketCount>capacity.activeMarketLimit;
  ...
  return <div><PageHeader eyebrow="PLAN & BILLING" title="Keep MarketRoute" accent="working for you." description="Choose the plan that matches how many markets you want MarketRoute working on at once."/>
    {campaignLimit&&<div className="mr-alert"><Icon name="campaigns" size={16}/><span>You have used all the market slots on your current plan. Upgrade here, then return to Add market — your brief stays saved.</span></div>}
    ...
    {billingAttention&&<div className="mr-alert mr-alert--danger"><Icon name="warning" size={16}/><span>Your subscription needs attention. Paid access is protected until billing is active again.</span></div>}
    {overCapacity&&<div className="mr-alert mr-alert--danger"><Icon name="warning" size={16}/><span>Your current plan allows {capacity.activeMarketLimit} active market{capacity.activeMarketLimit===1?"":"s"}, but this workspace has {capacity.activeMarketCount}. MarketRoute has paused extra market research without deleting anything. Archive markets or upgrade to continue.</span></div>}
    {showCurrent&&<div className="mr-plan-current"><Icon name="shield" size={18}/><div><span>YOUR PLAN</span><strong>{access.planName??context.planCode??"MarketRoute subscription"}</strong><p>{context.cancelAtPeriodEnd&&context.currentPeriodEnd?`Your subscription remains active until ${new Date(context.currentPeriodEnd).toLocaleDateString("en-GB")}.`:billingAttention?"Update your payment method or subscription in the secure billing portal.":legacyFull?"This workspace was grandfathered during the billing migration. You can keep this access or move onto a standard MarketRoute subscription below.":"Your opportunities are unlocked and MarketRoute will keep researching within this plan."}</p>{context.externalCustomerId&&<form action="/api/billing/portal" method="post"><button className="mr-button mr-button--secondary" type="submit">Manage billing <Icon name="arrow" size={14}/></button></form>}</div></div>}
    {showChooser&&<div className="mr-plans-page__chooser"><PlanChooser plans={eligibleLegacyPlans} lockedCount={access.lockedCount} title={legacyFull?"Choose a MarketRoute plan":"Choose the right level of MarketRoute"}/>{legacyFull&&<p className="mr-plans-page__legacy-note">Your grandfathered access stays active unless you complete checkout. Moving to a standard subscription replaces the legacy entitlement with that plan.</p>}</div>}
  </div>;
}
