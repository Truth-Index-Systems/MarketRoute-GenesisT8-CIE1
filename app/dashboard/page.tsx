...
function iconFor(key:string):IconName{return key==="bootstrap"?"spark":key==="discovery"?"companies":key==="research"?"research":key==="evidence"?"database":key==="truth"?"shield":key==="r4"?"check":key==="r5"?"route":key==="r6"?"user":key==="opportunity"?"opportunities":key==="engagement"?"engagement":"mail";}
...
export default async function FounderDashboard(){
  ...
  const platform=obj(snapshot.platform),research=obj(snapshot.research),evidence=obj(snapshot.evidence),truth=obj(snapshot.truth),r4=obj(snapshot.r4),r5=obj(snapshot.r5),r6=obj(snapshot.r6),opportunity=obj(snapshot.opportunity),engagement=obj(snapshot.engagement),ai=obj(snapshot.ai),runtime=obj(snapshot.runtime),schema=obj(snapshot.schemaRelease);
  ...
  return <main className="mr-founder">
    ...
    <section className="mr-founder__two-col">
      <div className="mr-founder-panel"><div className="mr-founder-panel__head"><div><span>DATA VOLUME</span><h2>What Genesis has accumulated</h2></div><Icon name="database" size={20}/></div><div className="mr-founder-data-list"><div><span>Canonical companies</span><strong>{integer(obj(snapshot.discovery).companies)}</strong></div><div><span>People</span><strong>{integer(obj(snapshot.discovery).people)}</strong></div><div><span>Sources</span><strong>{integer(evidence.sources)}</strong></div><div><span>Acquisitions</span><strong>{integer(evidence.acquisitions)}</strong></div><div><span>Evidence items</span><strong>{integer(evidence.items)}</strong></div><div><span>Claims</span><strong>{integer(evidence.claims)}</strong></div><div><span>Truth entity snapshots</span><strong>{integer(truth.entitySnapshots)}</strong></div><div><span>Commercial relationships</span><strong>{integer(r5.relationships)}</strong></div><div><span>Graph nodes</span><strong>{integer(r5.graphNodes)}</strong></div></div></div>

      <div className="mr-founder-panel"><div className="mr-founder-panel__head"><div><span>PRODUCTION RUNTIME</span><h2>Cron heartbeat</h2></div><Icon name="clock" size={20}/></div><div className="mr-founder-runtime">{([['BOOTSTRAP','Every 10 min'],['RESEARCH','Every 5 min'],['DELIVERY','Every 2 min']] as const).map(([key,schedule])=>{const row=obj(runtime[key]);const event=text(row.eventType,"NO RUN");const at=typeof row.occurredAt==="string"?row.occurredAt:null;return <div key={key}><span className={`mr-founder-runtime__dot mr-founder-runtime__dot--${event.toLowerCase()}`}/><div><strong>{key.charAt(0)+key.slice(1).toLowerCase()}</strong><small>{schedule}</small></div><div className="mr-founder-runtime__right"><strong>{event}</strong><small>{relative(at)}</small></div></div>})}</div><div className="mr-founder-runtime-note">Delivery is {environment.deliveryEnabled?<strong>enabled</strong>:<strong>intentionally disabled</strong>}. A disabled delivery heartbeat is healthy while the kill switch remains off.</div></div>
    </section>
