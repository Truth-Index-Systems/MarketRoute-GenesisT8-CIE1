...
export function ContactRouteCard({route,returnHref,companyWebsiteHref,index}:{route:ContactRouteView;returnHref:string;companyWebsiteHref:string|null;index:number}){
  ...
  const role=route.roleTitles.length?route.roleTitles.join(" · "):route.personName?"Current role verified":"Direct organisational access";
  ...
  return <article className="mr-contact-route-card">
    <header><div className="mr-contact-route-card__identity"><div className="mr-contact-route-card__avatar"><Icon name={route.personName?"user":"companies"} size={19}/></div><div><span>{`READY ROUTE ${index+1}`}</span><h3>{title}</h3><p>{role}</p></div></div><div className="mr-contact-route-card__ready"><i/>Ready</div></header>
    <div className="mr-contact-route-card__why"><Icon name="route" size={15}/><div><strong>Why this route</strong><p>{route.explanation}</p></div></div>
    ...
    {(profiles.length>0||web.length>0||companyWebsiteHref)&&<div className="mr-contact-route-card__links">
      {profiles.map(channel=><span key={`${channel.key}:${channel.value}`}>{external(channel,"Open professional profile")}{channel.evidenceSnapshotId&&evidenceLink(channel.evidenceSnapshotId,returnHref,"Evidence")}</span>)}
      {web.map(channel=><span key={`${channel.key}:${channel.value}`}>{external(channel,channel.sourceKind.includes("FORM")?"Open contact form":"Open route")}{channel.evidenceSnapshotId&&evidenceLink(channel.evidenceSnapshotId,returnHref,"Evidence")}</span>)}
    ...
    <footer><div><Icon name="clock" size={13}/><span>{freshUntil?`Current until ${formatDateTime(freshUntil)}`:"Current authority is being monitored"}</span></div><div>{evidenceLink(route.identityEvidenceSnapshotId,returnHref,"Identity")}{evidenceLink(route.employmentEvidenceSnapshotId,returnHref,"Employment")}{evidenceLink(route.roleEvidenceSnapshotId,returnHref,"Role")}</div></footer>
  </article>;
}
