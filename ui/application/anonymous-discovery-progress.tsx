...
function OpportunityCard({opportunity,onGate}:{opportunity:AnonymousOpportunityPreview;onGate:()=>void}){
  ...
  return <article className="mr-discovery-opportunity">
    <header><div><span>FREE OPPORTUNITY {opportunity.ordinal}</span><h3>{opportunity.companyName}</h3><small>{opportunity.canonicalDomain??"Company website confirmed"}</small></div><div className="mr-discovery-opportunity__status"><i/>Ready</div></header>
    <p className="mr-discovery-opportunity__summary">{opportunity.narrative.summary}</p>
    {primary?<div className="mr-discovery-route-preview"><div className="mr-discovery-route-preview__identity"><div><Icon name={primary.personName?"user":"companies"} size={17}/></div><span><strong>{primary.personName??"Company route"}</strong><small>{primary.roleTitles.length?primary.roleTitles.join(" · "):"Direct route in"}</small></span></div><p>{primary.explanation}</p><div className="mr-discovery-route-preview__channels">
      {primary.emailAvailable?<GateButton label="Reveal email" onOpen={onGate} icon="mail"/>:<span><Icon name="mail" size={13}/>Email not found yet</span>}
      {primary.phoneAvailable?<GateButton label="Reveal phone" onOpen={onGate} icon="phone"/>:<span><Icon name="phone" size={13}/>Phone not found yet</span>}
    ...
    {opportunity.routes.length>1&&<footer>{opportunity.routes.length-1} additional contact route{opportunity.routes.length===2?"":"s"} will be saved with this opportunity.</footer>}
  </article>;
}
...
export function AnonymousDiscoveryProgress({initial}:{initial:AnonymousDiscoveryView}){
  ...
  return <div className="mr-discovery-progress" aria-live="polite">
    ...
    <section className="mr-discovery-free-eight">
      <div className="mr-discovery-free-eight__head"><div><span>YOUR FIRST 8</span><h2>Opportunities that are yours to keep.</h2><p>As soon as MarketRoute has a strong reason to pursue a company and a usable contact route, it becomes one of your eight permanent free opportunities.</p></div><strong>{state.opportunities.length} / {state.freeOpportunityLimit}</strong></div>
      {state.opportunities.length?<div className="mr-discovery-opportunity-grid">{state.opportunities.map(opportunity=><OpportunityCard key={opportunity.opportunityId} opportunity={opportunity} onGate={()=>setGateOpen(true)}/>)}</div>:<div className="mr-discovery-free-eight__waiting"><Icon name="search" size={18}/><div><strong>I’m still checking the first opportunity.</strong><span>The cards will appear here automatically as soon as the research is strong enough.</span></div></div>}
    </section>

    <section className="mr-discovery-live"><div><span>LIVE PROGRESS</span><h2>What MarketRoute has found so far</h2></div><dl><div><dt>Companies found</dt><dd>{state.metrics.scopedCompanies}</dd></div><div><dt>Companies researched</dt><dd>{state.metrics.researchedCompanies}</dd></div><div><dt>Opportunities found</dt><dd>{state.metrics.opportunities}</dd></div><div><dt>Your free opportunities</dt><dd>{state.metrics.freeUnlocked}</dd></div></dl><footer><Icon name="shield" size={14}/><span>This research is saved and continues automatically. Your first eight opportunities stay yours.</span></footer></section>

    {gateOpen&&<div className="mr-discovery-gate" role="dialog" aria-modal="true" aria-labelledby="mr-discovery-gate-title"><button className="mr-discovery-gate__backdrop" aria-label="Close" onClick={()=>setGateOpen(false)}/><div className="mr-discovery-gate__panel"><button className="mr-discovery-gate__close" onClick={()=>setGateOpen(false)} aria-label="Close">×</button><div className="mr-kicker"><span/> SAVE YOUR MARKETROUTE</div><h2 id="mr-discovery-gate-title">These opportunities are yours. Save them.</h2><p>I’ve built this MarketRoute specifically for {state.companyName}. Create a free account to keep the research and reveal the contact actions on your first eight opportunities.</p><ul><li><Icon name="check" size={13}/>Nothing starts again</li><li><Icon name="check" size={13}/>Your first eight stay free</li><li><Icon name="check" size={13}/>Email, phone and direct contact actions unlock after sign-in</li></ul><a className="mr-button mr-button--primary" href="/signup?claim=discovery&next=%2Fapp%2Fopportunities">Create free account <Icon name="arrow" size={15}/></a><a className="mr-discovery-gate__signin" href="/login?claim=discovery&next=%2Fapp%2Fopportunities">Already have an account? Sign in and save these opportunities.</a></div></div>}
  </div>;
}
