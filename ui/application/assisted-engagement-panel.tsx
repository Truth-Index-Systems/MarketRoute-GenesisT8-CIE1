...
export function AssistedEngagementPanel({engagement,opportunityId,pathFingerprint,returnHref,canMutate,paidAccess}:{engagement:JsonObject;opportunityId:string|null;pathFingerprint:string|null;returnHref:string;canMutate:boolean;paidAccess:boolean}){
  ...
  return <Panel className="mr-assisted-engagement-panel">
    ...
    {messageId&&<div className="mr-assisted-draft">
      <div className="mr-assisted-draft__meta"><div><span>CHANNEL</span><strong>{humanStatus(channel||"Prepared")}</strong></div><div><span>MESSAGE CHECK</span><StatusBadge compact label={humanStatus(reviewVerdict)} tone={reviewVerdict==="PASS"?"green":reviewVerdict==="BLOCK"?"red":"amber"}/></div><div><span>YOUR APPROVAL</span><strong>{approved?"Approved":text(approval.decision,"Pending")}</strong></div></div>
      ...
      {approved&&!contacted&&<div className="mr-assisted-action-desk">
        <div><span>YOUR ACTION</span><strong>Contact them yourself</strong><p>MarketRoute never sends this automatically.</p></div>
