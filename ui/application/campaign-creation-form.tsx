...
export function CampaignCreationForm({capacity,plans,processing=false,initialPaywallOpen=false}:{capacity:CampaignCapacity;plans:PublicPlan[];processing?:boolean;initialPaywallOpen?:boolean}){
  ...
  const capacityCopy=capacity.mode==="FULL"
    ? "Grandfathered full access is active. This market can start now; standard plan limits apply only if you move to a subscription."
    : capacity.mode==="DISCOVERY_FREE"
      ? "Your original Discovery market is preserved. Configure this new market first; MarketRoute will ask you to choose access only when you start it."
      : capacity.mode==="UNENTITLED"
        ? "Configure the market first. MarketRoute will ask you to choose a plan only when you start it."
        : gated
  ...
  return <>
    <section className="mr-campaign-entry-card">
      ...
      <p>Tell MarketRoute what you sell, who you want to reach and what success looks like. It will structure the research for you.</p>
      ...
      <form action="/api/campaigns" method="post" className="mr-campaign-entry-form" onSubmit={submit}>
        ...
        <label><span>Market name</span><input name="campaignName" required minLength={3} maxLength={120} autoComplete="off" placeholder="UK operations leaders" value={draft.campaignName} onChange={field("campaignName")}/><small>Use a name you will recognise later.</small></label>
        <label><span>What do you sell?</span><textarea name="sellerOfferingText" required minLength={8} maxLength={2000} rows={4} placeholder="Handover and shift-continuation software for operational teams." value={draft.sellerOfferingText} onChange={field("sellerOfferingText")}/><small>MarketRoute uses this to understand the offer it is taking to market.</small></label>
        ...
        <label><span>What outcome do you want?</span><textarea name="objectiveText" required minLength={8} maxLength={2000} rows={3} placeholder="Find organisations with a strong reason to improve shift handovers and a route to the operational buyer." value={draft.objectiveText} onChange={field("objectiveText")}/></label>
        ...
        <div className={`mr-campaign-advanced ${advanced?"is-open":""}`}>
          ...
          <button type="button" className="mr-campaign-advanced__toggle" onClick={toggleAdvanced} aria-expanded={advanced}>
            <span><Icon name="shield" size={15}/><b>{advanced?"Non-negotiables added":"Add non-negotiables"}</b><small>Optional · geography, company type or other non-negotiables</small></span>
            <strong>{advanced?"−":"+"}</strong>
          </button>
          {advanced&&<label className="mr-campaign-advanced__field"><span>Non-negotiables</span><textarea name="hardConstraintsText" required minLength={3} maxLength={2000} rows={3} placeholder="For example: UK only; B2B only; 50–1,000 employees." value={draft.hardConstraintsText} onChange={field("hardConstraintsText")}/><small>MarketRoute treats these as firm limits, not preferences.</small></label>}
        </div>

        {processing&&<div className="mr-alert"><Icon name="clock" size={16}/><span>MarketRoute is already preparing another market. This brief is saved and can start when that finishes.</span></div>}
        <button className="mr-button mr-button--primary mr-campaign-entry-submit" type="submit" disabled={processing}>{gated?"Continue to plan options":"Start this market"} <Icon name="arrow" size={18}/></button>
        <small className="mr-campaign-entry-foot">MarketRoute reuses useful research where it can and fills the gaps for this market.</small>
      </form>
    </section>

    {paywallOpen&&<div className="mr-commercial-modal" role="dialog" aria-modal="true" aria-label="Market plan limit"><button className="mr-commercial-modal__backdrop" type="button" onClick={()=>setPaywallOpen(false)} aria-label="Close market access"/><div className="mr-commercial-modal__panel"><div className="mr-campaign-paywall"><header><div><span>ADD ANOTHER MARKET</span><h2>Work another market at the same time.</h2><p>{capacity.activeMarketCount>0?`You currently have ${capacity.activeMarketCount} active market${capacity.activeMarketCount===1?"":"s"}. Choose a plan that can accommodate market ${capacity.activeMarketCount+1}.`:"Additional markets are a paid MarketRoute capability. Your configured brief stays saved while you choose access."}</p></div><button type="button" onClick={()=>setPaywallOpen(false)} aria-label="Close">×</button></header>
      {eligiblePlans.length>0?<div className="mr-campaign-paywall__plans">{eligiblePlans.map(plan=><article key={plan.planCode} className={plan.planCode===capacity.nextPlanCode?"is-next":""}>{plan.planCode===capacity.nextPlanCode&&<b>BEST FIT</b>}<span>{plan.displayName}</span><strong>£{Math.round(plan.monthlyPriceGbp)}<small>/month</small></strong><p>{marketLabel(plan.activeMarketLimit)}</p>{paid?null:<form action="/api/billing/checkout" method="post"><input type="hidden" name="planCode" value={plan.planCode}/><button className="mr-button mr-button--primary" type="submit">Choose {plan.displayName} <Icon name="arrow" size={14}/></button></form>}</article>)}</div>:<div className="mr-alert"><Icon name="shield" size={16}/><span>Your current self-serve plan range is full. Archive an active market or contact support for a larger workspace.</span></div>}
      {paid&&eligiblePlans.length>0&&<form action="/api/billing/portal" method="post" className="mr-campaign-paywall__portal"><button className="mr-button mr-button--primary" type="submit">Upgrade current plan <Icon name="arrow" size={14}/></button><small>Plan changes are completed securely in the billing portal. Your market brief stays saved on this device.</small></form>}
      {eligiblePlans.length===0&&<a className="mr-button mr-button--secondary" href="/support">Contact support</a>}
      <footer><Icon name="shield" size={14}/><span>Plan limits change how many markets MarketRoute can work at once. Your existing research stays intact.</span></footer></div></div></div>}
  </>;
}
