...
- `0058_market_map_opportunity_read_performance_hotfix.sql` — replaces Market Map and Opportunities list-time per-company authority recomputation with bounded materialised summary projections over indexed campaign scope, persisted R4/R5/R6 lineage and canonical opportunity-sync snapshots. Exact current authority remains mandatory on company/opportunity detail and engagement execution paths.
- `0063_locked_opportunity_projection_bootstrap_performance_hardening.sql` — removes exact per-row authority recomputation from Discovery entitlement/refill list surfaces, restores the two locked teasers from the bounded materialised current-sync projection, and keeps exact authority gates on detail/engagement paths.
- `0064_paid_campaign_refill_claim_ambiguity_hotfix.sql` — removes PL/pgSQL output-column ambiguity from paid refill claiming while preserving the existing bounded refill policy.
- `0065_human_discovery_demand_fed_genesis_bank.sql` — lets customer-driven discovery feed canonical company identities back into the reusable Genesis bank, allows identity-known bank reuse ordered behind denser global research, backfills modern activation lineage, and promotes no customer-private evidence or authority.
