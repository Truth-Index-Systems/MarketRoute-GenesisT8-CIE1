...
check("post-freeze operational migrations are explicitly declared",()=>{assert.equal(release.databaseMigrationRequired,true);assert.equal(release.latestOperationalMigration,65);assert(Array.isArray(release.operationalMigrations)&&release.operationalMigrations.length>=2);assert(release.operationalMigrations.every(m=>m.authorityWriter===false));});
