...
const missing=[];
for(const key of ["SUPABASE_URL","OPENAI_API_KEY","CRON_SECRET","FOUNDER_DASHBOARD_PASSWORD","FOUNDER_DASHBOARD_SESSION_SECRET","MARKETROUTE_ANONYMOUS_SESSION_SECRET"])if(!present(key))missing.push(key);
...
if(String(process.env.MARKETROUTE_ANONYMOUS_SESSION_SECRET).trim().length<32)throw new Error("MARKETROUTE_ANONYMOUS_SESSION_SECRET must be at least 32 characters");
...
console.log(`MarketRoute production environment contract: PASS (Supabase=${mode}, model=${process.env.OPENAI_MODEL||"gpt-5.6-luna"}, delivery=assisted-only)`);
