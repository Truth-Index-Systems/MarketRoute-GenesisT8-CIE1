...
check("Build 18 package version is frozen candidate",()=>assert.equal(pkg.version,"0.18.0"));
...
check("Build 18 adds no schema migration",()=>{assert.equal(release.databaseMigrationRequired,false);assert.equal(Math.max(...migrations.map(f=>Number(path.basename(f).slice(0,4)))),19);});
