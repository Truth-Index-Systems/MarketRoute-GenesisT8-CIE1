...
check("Build 18 certification remains schema-free and post-freeze operational migrations are isolated",()=>{assert.equal(release.databaseMigrationRequired,false);const max=Math.max(...migrations.map(f=>Number(path.basename(f).slice(0,4))));assert([19,20,21,22,23,24].includes(max));if(max>=20){const activation=read("supabase/migrations/0020_production_activation_runtime.sql");assert(/production activation/i.test(activation));assert(/new_authority_writer[^\n]*false/i.test(activation));assert(!/INSERT\s+INTO\s+public\.(?:commercial_reality_r4_records|route_authority_r5_records|contact_authority_r6_records|authority_records)/i.test(activation));}if(max>=21){const founder=read("supabase/migrations/0021_founder_dashboard_observability.sql");assert(/founder dashboard|founder observability/i.test(founder));assert(/new_authority_writer[^\n]*false/i.test(founder));assert(!/INSERT\s+INTO\s+public\.(?:commercial_reality_r4_records|route_authority_r5_records|contact_authority_r6_records|authority_records)/i.test(founder));}if(max>=22){const growth=read("supabase/migrations/0022_genesis_database_growth.sql");assert(/DATABASE_GROWTH|database growth/i.test(growth));assert(/new_authority_writer[^\n]*false/i.test(growth));assert(!/INSERT\s+INTO\s+public\.(?:commercial_reality_r4_records|route_authority_r5_records|contact_authority_r6_records|authority_records)/i.test(growth));}if(max>=23){const hotfix=read("supabase/migrations/0023_production_runtime_ambiguity_cost_hotfix.sql");assert(/production hotfix 0\.18\.3\.[12]/i.test(hotfix));assert(!/INSERT\s+INTO\s+public\.(?:commercial_reality_r4_records|route_authority_r5_records|contact_authority_r6_records|authority_records)/i.test(hotfix));}if(max>=24){const truthHotfix=read("supabase/migrations/0024_truth_entity_snapshot_ambiguity_hotfix.sql");assert(/production hotfix 0\.18\.3\.3/i.test(truthHotfix));assert(/tes\.input_fingerprint\s*=\s*v_input_fingerprint/i.test(truthHotfix));assert(!/INSERT\s+INTO\s+public\.(?:commercial_reality_r4_records|route_authority_r5_records|contact_authority_r6_records|authority_records)/i.test(truthHotfix));}});
