...
console.log("\nAWS V0 Build 7 — AI boundary static gate through 7.5");
...
const definition = read("core/ai/semantic-probe-definition.ts");
...
check("provider-neutral operation definition exists", () => assert(definition.includes("SEMANTIC_PROBE_JSON_SCHEMA") && definition.includes("parseSemanticProbeOutput"), "semantic definition missing"));
...
check("retry policy is explicit", () => assert(application.includes("maxAttempts: 2") && application.includes("retryDelayMs: 150"), "default retry policy missing"));
...
check("Bedrock SDK is confined to AWS platform adapter", () => assert(bedrockRuntime.includes('@aws-sdk/client-bedrock-runtime') && bedrockRuntime.includes('import "server-only"'), "server-only SDK boundary missing"));
check("7.5 activates only non-streaming Converse", () => {
  assert(bedrockProvider.includes("ConverseCommand"), "ConverseCommand missing");
  assert(bedrockProvider.includes("this.client.send(command, { abortSignal: signal })"), "controlled Bedrock send missing");
  for (const forbidden of ["ConverseStreamCommand", "InvokeModelCommand", "InvokeModelWithResponseStreamCommand"]) {
    assert(!bedrockProvider.includes(forbidden), `forbidden command: ${forbidden}`);
  }
});
check("Bedrock structured output is schema constrained", () => {
  for (const token of ["outputConfig", "textFormat", 'type: "json_schema"', "SEMANTIC_PROBE_JSON_SCHEMA", "parseStructuredText"]) {
    assert(bedrockProvider.includes(token), `structured output token missing: ${token}`);
  }
});
check("no canonical persistence exists in AI boundary", () => {
  const combined = [contract, definition, application, provider, bedrockRuntime, bedrockProvider].join("\n").toLowerCase();
...
check("no deterministic commercial authority is added", () => {
  const lower = [contract, definition].join("\n").toLowerCase();
...
else console.log("\nPASS  Build 7 AI boundary through 7.5");
